...
class CarryingBill < ActiveRecord::Base
  ...
  #今日收货
  scope :today_billed,lambda {|from_org_ids| select('pay_type,sum(carrying_fee) carrying_fee,sum(goods_fee) goods_fee,sum(goods_num) goods_num,sum(1) as bill_count').where(:from_org_id => from_org_ids,:bill_date => Date.today).group(:pay_type)}
  #今日提货
